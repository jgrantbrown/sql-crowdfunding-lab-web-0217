...
"SELECT projects.title, (SUM(pledges.amount)-projects.funding_goal) AS over_sold FROM projects JOIN pledges ON projects.id = pledges.project_id GROUP BY projects.title HAVING over_sold>=0 "
end
